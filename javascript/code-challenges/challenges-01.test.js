...
const addOne = (arr) => {
  let out = [];
  arr.forEach((num) => out.push(num + 1));
  return out;
};
...
const allUpperCase = (arr) => {
  let out = [];
  arr.forEach((str) => out.push(str.toUpperCase()));
  return out;
};
...
const createList = (availableItems) => {
  let list = [];
  availableItems.forEach((item) => item.available === true ? list.push(item.name) : console.log('unavailable'));
  return list;
};
