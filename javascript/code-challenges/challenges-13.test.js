...
const longestString = (arr) => {
  ...
  let longest = Math.max.apply(null, lengths);
  return lengths.findIndex(len => len === longest);
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.every(str => /(:\))/g.test(str));
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map((str, idx) => str[idx]);
};
