...
const count = (target, input) => {
  return input.reduce((acc, itm) => {
    if (typeof itm === 'number' && itm === target) acc += 1;
    else if (typeof itm === 'object') acc += count(target, itm);
    return acc;
  }, 0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {
    return arr.filter(itm => {
      return typeof itm === 'number' && itm % 5 === 0;
    })
  }).map(arr => arr.map(num => Math.pow(2, num)))
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.reduce((acc, obj, idx, th) => {
    if (obj.gender === 'male' || obj.gender === 'female') {
      acc += obj.name;
      if (idx !== th.length - 1) acc += ' and ';
    }
    return acc;
    }, '');
};
...
let findShortest = (data) => {
  return data.reduce((acc, obj, idx, th) => {
    let last = th.find(obj => obj.name === acc);
    if (parseInt(obj.height) < last.height) {
      acc = obj.name;
    }
    return acc;
  }, data[0].name)
