...
const typeNum = (arr) => {
  return arr.filter(itm => typeof itm === 'number');
};
...
const containsAnd = (arr) => {
  return arr.filter(itm => {
    return (typeof itm === 'string') && itm.includes('and');
  });
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.reduce((acc, itm) => {
    if (itm.baseStat > minBaseStat) acc.push(itm.stat.name);
    return acc;
  }, []);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(people => !people.hasOwnProperty('children'));
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(itm => typeof itm === 'number').map(num => num % 2 === 0 ? 'even' : 'odd');
};
